  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      for (let i = 0; i < words.length; i++) {
        ...
        setMessages((prev) =>
          prev.map((msg) => (msg._id === aiMessage._id ? { ...msg, text: currentText } : msg))
        );
      }
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'row', gap: '20px', height: '80vh' }}>
        ...
        >
          <h2
            style={{ textAlign: 'center', marginBottom: '25px', color: '#333', fontWeight: '700' }}
          >
            👋 {username}
          </h2>

          <button onClick={() => (window.location.href = '/profile')} style={buttonStyle}>
            Profile
          </button>
          <button onClick={() => (window.location.href = '/dashboard')} style={buttonStyle}>
            Dashboard
          </button>
          <button onClick={handleNewChat} style={{ ...buttonStyle, backgroundColor: '#57bcff' }}>
            + New Chat
          </button>
            ...
            {chatHistory.map((chat) => (
              <div
                ...
                style={{
                  ...
                  marginBottom: '8px',
                  borderRadius: '10px',
                  backgroundColor: currentChatId === chat.id ? '#dbeafe' : '#f5f5f5',
                  cursor: 'pointer',
                  fontSize: '14px',
                }}
              >
