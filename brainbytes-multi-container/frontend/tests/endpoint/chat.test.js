...
beforeAll(async () => {
  try {
    ...
    console.warn(`Backend not reachable at ${API_URL} — skipping endpoint tests`);
  }
});

const requireBackend = (fn) => async () => {
  if (!backendAvailable) {
    return;
  }
  await fn();
...
describe('E2E: Backend API reachable from frontend container', () => {
  test('backend root responds with welcome message', requireBackend(async () => {
    const res = await axios.get(`${API_URL}/`);
    expect(res.status).toBe(200);
    expect(res.data.message).toBe('Welcome to the BrainBytes API');
  }));

  test('can fetch messages list', requireBackend(async () => {
    const res = await axios.get(`${API_URL}/api/messages`);
    expect(res.status).toBe(200);
    expect(Array.isArray(res.data)).toBe(true);
  }));

  test('can post a message and receive it back', requireBackend(async () => {
    const payload = {
  ...
  test('can fetch materials list', requireBackend(async () => {
